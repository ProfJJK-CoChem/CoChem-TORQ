...
Enforces MolSym tarball extraction per user specifications.
"""
import os
import sys
import subprocess
...
class DynamicSiloManager:
    def __init__(self, project_name: str):
        ...
        if self.main_env not in env_list:
            subprocess.check_call([self.conda, "create", "-y", "-n", self.main_env, "python=3.12", "ipykernel"], stdout=subprocess.DEVNULL)
    ...
    def enforce_molsym(self) -> None:
        """Strictly installs MolSym from a tar.gz package."""
        ...
        tarballs = glob.glob("MolSym*.tar.gz")
        ...
        print_status("1. Go to: https://github.com/NASymmetry/MolSym", "warning")
        print_status("2. Download: MolSym-1.0.0.tar.gz", "warning")
...
def main() -> None:
    ...
    try:
        with open("cochem_state_p3.json", "r") as f: state = json.load(f)
    except FileNotFoundError:
        print_status("Missing cochem_state_p3.json.", "fail")
        sys.exit(1)
        
    manager = DynamicSiloManager(project_name="TOPOS")
    ...
    with open("cochem_state_p4.json", "w") as f: json.dump(state, f, indent=4)
